...
namespace GrabAccess
{
    ...
    namespace
    {
        ...
        static void CollectAccessTargets(SDK::UWorld* pGWorld, std::vector<SDK::AActor*>& out)
        {
            ...
            if (auto* pKey = HeistUtil::FindClass("BP_KeycardBase_C"))
                pushFiltered(pKey, false);
            if (auto* pRfid = HeistUtil::FindClass("BP_RFIDTagBase_C"))
                pushFiltered(pRfid, false);
            if (auto* pBadge = HeistUtil::FindClass("BP_Chus_CarriedPressBadge_C"))
