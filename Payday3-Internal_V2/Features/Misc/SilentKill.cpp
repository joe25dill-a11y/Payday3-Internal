...
namespace SilentKill
{
    ...
    namespace
    {
        ...
        static int RunWave(SDK::UWorld* pGWorld)
        {
            ...
            SDK::UClass* pCopClass = HeistUtil::FindClass("CH_BaseCop_C");
            if (!pCopClass)
                pCopClass = SDK::ASBZAICharacter::StaticClass();
            if (!pCopClass)
                return 0;
            ...
            for (int i = 0; i < list.Num() && killed < kMaxPerWave; ++i)
            {
                ...
                if (auto* pCiv = HeistUtil::FindClass("CH_BaseCivilian_C"); pCiv && pActor->IsA(pCiv))
                {
                    ++skipped;
                    continue;
                }
