...
namespace PresetTeleport
{
    ...
    void SetSelectedIndex(int index)
    {
        ...
        if (index < 0)
            index = 0;
        if (index >= (int)s_vecSpots.size())
            index = (int)s_vecSpots.size() - 1;
        s_bTour = false;
        s_iIndex = index;
        s_iLastCrewTp = 0;
        RefreshStatus();
    ...
    {
        if (index < 0 || index >= (int)s_vecSpots.size())
            return {};
        return s_vecSpots[index].name;
    }
